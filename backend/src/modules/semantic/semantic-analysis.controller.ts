...
import { SemanticAnalysisService, ContentInjectionResult, PreferencePattern } from './semantic-analysis.service';
...
export class SemanticAnalysisController {
  ...
  @ApiOperation({
    ...
    description: 'Analiza patrones de preferencias y añade contenido relevante cuando hay pocos matches'
  })
  @ApiResponse({
    status: 200,
  ...
  async injectSemanticContent(
    ...
    this.logger.log(`🎯 User ${req.user.sub} requesting content injection for room ${roomId}`);
    
    // Verificar si la sala necesita inyección de contenido
    const shouldInject = await this.semanticAnalysisService.shouldInjectContent(roomId);
    
    if (!shouldInject) {
      this.logger.log(`❌ Room ${roomId} does not meet criteria for content injection`);
      return {
        injectedContent: [],
        analysisMetadata: {
          patternsFound: await this.semanticAnalysisService.analyzePreferencePatterns(roomId),
    ...
    return this.semanticAnalysisService.injectSemanticContent(roomId, dto.maxInjections);
  }

  @Get('rooms/:roomId/preferences')
  @ApiOperation({
    summary: 'Analizar patrones de preferencias de una sala',
    description: 'Obtiene análisis detallado de los patrones de votación positiva de los miembros'
  })
  @ApiResponse({
    status: 200,
  ...
  async analyzePreferences(
    ...
    this.logger.log(`📊 User ${req.user.sub} analyzing preferences for room ${roomId}`);
  ...
  @ApiOperation({
    ...
    description: 'Evalúa criterios para determinar si se debe inyectar contenido semántico'
  })
  @ApiResponse({
    status: 200,
  ...
  async getInjectionStatus(
    ...
    this.logger.log(`🔍 User ${req.user.sub} checking injection status for room ${roomId}`);
  ...
  @ApiOperation({
    ...
    description: 'Inyecta contenido semántico sin verificar criterios automáticos'
  })
  @ApiResponse({
    status: 200,
  ...
  async forceInjectContent(
    ...
    this.logger.log(`⚡ User ${req.user.sub} forcing content injection for room ${roomId}`);

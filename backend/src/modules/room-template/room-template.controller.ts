...
export class RoomTemplateController {
  ...
  async createTemplate(@Request() req, @Body() createTemplateDto: CreateTemplateDto) {
    return this.roomTemplateService.createTemplate(req.user.id, createTemplateDto);
  }

  @Get()
  @ApiOperation({ summary: 'Obtener plantillas públicas con filtros opcionales' })
  ...
  @ApiQuery({ name: 'sortBy', required: false, description: 'Ordenar por campo' })
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Número de plantillas a retornar (default: 10)' })
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Actualizar una plantilla existente (solo el creador)' })
  @ApiResponse({ status: 200, description: 'Plantilla actualizada exitosamente' })
  @ApiResponse({ status: 403, description: 'Solo el creador puede actualizar la plantilla' })
  @ApiResponse({ status: 404, description: 'Plantilla no encontrada' })
  async updateTemplate(
    ...
    return this.roomTemplateService.updateTemplate(templateId, req.user.id, updateTemplateDto);
  }
  ...
  @ApiResponse({ status: 403, description: 'Solo el creador puede eliminar la plantilla' })
  @ApiResponse({ status: 404, description: 'Plantilla no encontrada' })
  ...
  @ApiResponse({ status: 201, description: 'Sala creada exitosamente desde plantilla' })
  @ApiResponse({ status: 403, description: 'No tienes acceso a esta plantilla' })
  @ApiResponse({ status: 404, description: 'Plantilla no encontrada' })
  async createRoomFromTemplate(
    ...
    return this.roomTemplateService.createRoomFromTemplate(templateId, req.user.id, createRoomDto);
  }

  @Post(':id/rate')
  @HttpCode(HttpStatus.NO_CONTENT)
  @ApiOperation({ summary: 'Calificar una plantilla' })
  @ApiResponse({ status: 204, description: 'Plantilla calificada exitosamente' })
  @ApiResponse({ status: 404, description: 'Plantilla no encontrada' })
  async rateTemplate(
    @Param('id') templateId: string,
    @Request() req,
    ...
    await this.roomTemplateService.rateTemplate(templateId, req.user.id, rateTemplateDto.rating);
  }

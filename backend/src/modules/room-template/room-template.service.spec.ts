...
describe('RoomTemplateService Property Tests', () => {
  ...
  const templateCategoryArb = fc.constantFrom(...Object.values(TemplateCategory));
  ...
  const advancedSettingsArb = fc.record({
  ...
  const templateConfigurationArb = fc.record({
    filters: fc.record({
      genres: fc.array(fc.string({ minLength: 3, maxLength: 20 }), { minLength: 1, maxLength: 5 }),
  ...
  const createTemplateDtoArb = fc.record({
    ...
    tags: fc.option(fc.array(fc.string({ minLength: 2, maxLength: 20 }).filter(s => s.trim().length >= 2), { maxLength: 10 })),
  });
  ...
  const roomTemplateArb = fc.record({
    ...
    tags: fc.array(fc.string({ minLength: 2, maxLength: 20 }).filter(s => s.trim().length >= 2), { maxLength: 10 }),
    ...
    it('should preserve all template data when creating a template', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (templateData, creatorId) => {
          ...
          // Act
          const result = await service.createTemplate(creatorId, templateData);
    ...
    it('should reject templates with invalid advanced settings', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (invalidTemplateData, creatorId) => {
          ...
          await expect(service.createTemplate(creatorId, invalidTemplateData))
            .rejects.toThrow(BadRequestException);
        }
      ), { numRuns: 50 }); // Menos iteraciones para casos específicos de error
    });
  });
    ...
    it('should calculate average rating correctly when adding new ratings', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (template, userId, newRating) => {
          ...
          const expectedRating = ((template.rating * template.ratingCount) + newRating) / expectedRatingCount;
          const expectedRatingRounded = Math.round(expectedRating * 100) / 100;
    ...
    it('should enforce access control for private templates', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (privateTemplate, creatorId, differentUserId) => {
          ...
          await expect(service.createRoomFromTemplate(templateWithCreator.id, creatorId))
            .resolves.toBeDefined();

          // Act & Assert - Un usuario diferente NO debe poder usar la plantilla
          if (differentUserId !== creatorId) {
            await expect(service.createRoomFromTemplate(templateWithCreator.id, differentUserId))
    ...
    it('should return templates that match search criteria', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (templates, searchQuery) => {
          // Arrange
          const publicTemplates = templates.map(t => ({ ...t, isPublic: true }));
          ...
          results.forEach(result => {
            const searchableText = `${result.name} ${result.description} ${result.tags.join(' ')}`.toLowerCase();
      ...
      ), { numRuns: 50 }); // Menos iteraciones para búsquedas complejas
    });
  });
            ...
            for (let i = 0; i < results.length - 1; i++) {
              ...
              switch (sortBy) {
                ...
                case 'rating':
                  ...
                  const currentRating = isNaN(current.rating) ? 0 : current.rating;
    ...
    it('should maintain data consistency across CRUD operations', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (templateData, creatorId) => {
          ...
          // Act - Create
          const created = await service.createTemplate(creatorId, templateData);
          ...
          const updated = await service.updateTemplate(created.id, creatorId, updateData);
      ...
      ), { numRuns: 50 }); // Menos iteraciones para operaciones con delay
    });
  });
        ...
        async (templates) => {
          ...
          if (popularTemplates.length > 1) {
            for (let i = 0; i < popularTemplates.length - 1; i++) {
              ...
              const currentScore = isNaN(current.popularityScore) ? 0 : current.popularityScore;
              const nextScore = isNaN(next.popularityScore) ? 0 : next.popularityScore;
          ...
          popularTemplates.forEach(template => {
            ...
            const expectedRatingScore = templateRating * template.ratingCount * 0.3;
            const expectedPopularityScore = expectedUsageScore + expectedRatingScore;
            
            const actualScore = isNaN(template.popularityScore) ? 0 : template.popularityScore;
  ...
  describe('Property 10: Template filtering produces correct subsets', () => {
    it('should filter templates correctly based on provided criteria', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (filterCategory, minRating) => {
          ...
          const validTemplates = Array.from({ length: 5 }, (_, index) => ({
            ...
            configuration: {
              filters: { genres: ['Action'], minYear: 2000, maxYear: 2024, minRating: null, includeAdult: false },
              settings: {} as any,
            },
            usageCount: index * 10,
      ...
      ), { numRuns: 50 }); // Menos iteraciones para filtros complejos
    });
  });
});
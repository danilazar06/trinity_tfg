...
export class RoomSettingsController {
  ...
  async getRoomSettings(
    ...
    const settings = await this.roomSettingsService.getRoomSettings(roomId, userId);
  ...
  @ApiOperation({ 
    ...
    description: 'Actualiza las configuraciones avanzadas de una sala. Solo administradores pueden realizar esta acción.'
  })
  @ApiParam({ name: 'roomId', description: 'ID de la sala' })
  @ApiResponse({ 
    status: 200, 
  ...
  async updateRoomSettings(
    ...
    const userId = req.user.sub;
    const settings = await this.roomSettingsService.updateRoomSettings(roomId, userId, updateDto);
  ...
  @ApiOperation({ 
    ...
    description: 'Restablece las configuraciones de la sala a los valores por defecto. Solo administradores pueden realizar esta acción.'
  })
  @ApiParam({ name: 'roomId', description: 'ID de la sala' })
  @ApiResponse({ 
    status: 200, 
  ...
  async resetRoomSettings(
    ...
    const userId = req.user.sub;
    const settings = await this.roomSettingsService.resetRoomSettings(roomId, userId);
  ...
  @ApiOperation({ 
    ...
    description: 'Obtiene recomendaciones inteligentes para optimizar las configuraciones de la sala basadas en su uso y características'
  })
  @ApiParam({ name: 'roomId', description: 'ID de la sala' })
  @ApiResponse({ 
    status: 200, 
  ...
  async getSettingsRecommendations(
    ...
    const userId = req.user.sub;
    return await this.roomSettingsService.getSettingsRecommendations(roomId, userId);
  }

  @Get('defaults')
  @ApiOperation({ 
    summary: 'Obtener configuraciones por defecto',
    description: 'Obtiene las configuraciones por defecto del sistema para referencia'
  })
  @ApiResponse({ 
    status: 200, 

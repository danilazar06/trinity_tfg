...
import { AdvancedRoomSettings, ConsensusType, RoomPrivacy } from '../../domain/entities/room-template.entity';
import { UpdateRoomSettingsDto, SettingsRecommendationDto, SettingsRecommendationsResponseDto } from './dto/room-settings.dto';
...
export class RoomSettingsService {
   ...
   */
  async getRoomSettings(roomId: string, userId: string): Promise<AdvancedRoomSettings> {
    ...
    try {
      ...
      if (!settingsEntity) {
        ...
        this.logger.log(`Devolviendo configuraciones por defecto para sala ${roomId}`);
        return DEFAULT_ROOM_SETTINGS;
      }

      return (settingsEntity as RoomSettingsEntity).settings;
    } catch (error) {
      this.logger.error(`Error obteniendo configuraciones de sala ${roomId}: ${error.message}`);
      throw error;
    }
  }
  ...
  async updateRoomSettings(
    ...
    if (!validationResult.isValid) {
      throw new BadRequestException(`Configuraciones inválidas: ${validationResult.errors.join(', ')}`);
    }
    ...
    try {
      ...
      this.logger.log(`Configuraciones actualizadas para sala ${roomId} por usuario ${userId}`);
      return newSettings;
    } catch (error) {
      this.logger.error(`Error actualizando configuraciones de sala ${roomId}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async resetRoomSettings(roomId: string, userId: string): Promise<AdvancedRoomSettings> {
    // Verificar que el usuario es admin de la sala
    await this.verifyRoomAdmin(roomId, userId);
    ...
    try {
      ...
      this.logger.log(`Configuraciones restablecidas para sala ${roomId} por usuario ${userId}`);
      return DEFAULT_ROOM_SETTINGS;
    } catch (error) {
      this.logger.error(`Error restableciendo configuraciones de sala ${roomId}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async getSettingsRecommendations(roomId: string, userId: string): Promise<SettingsRecommendationsResponseDto> {
    ...
    try {
      ...
      const recommendations = await this.generateRecommendations(room, currentSettings);
      
      // Calcular puntuaciones de optimización
      const currentScore = this.calculateOptimizationScore(currentSettings, room);
      const potentialScore = this.calculatePotentialScore(currentSettings, recommendations, room);
      ...
      this.logger.error(`Error generando recomendaciones para sala ${roomId}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  private async validateSettings(settings: UpdateRoomSettingsDto): Promise<{ isValid: boolean; errors: string[] }> {
    const errors: string[] = [];
    ...
    try {
      ...
      if (settings.anonymousVoting && settings.showVotingProgress) {
        errors.push('No se puede mostrar progreso de votación con votación anónima habilitada');
      }
  ...
  private async generateRecommendations(
    ...
    if (memberCount > 15 && currentSettings.consensusThreshold === ConsensusType.UNANIMOUS) {
      recommendations.push({
        setting: 'consensusThreshold',
        recommendedValue: ConsensusType.MAJORITY,
        reason: 'Con muchos miembros, el consenso unánime puede ser difícil de alcanzar',
        priority: 4,
    ...
    if (currentSettings.votingTimeout && currentSettings.votingTimeout < 45 && memberCount > 8) {
      recommendations.push({
    ...
    if (!currentSettings.contentInjectionEnabled) {
      recommendations.push({
        ...
        reason: 'La inyección de contenido mejora la diversidad y descubrimiento',
        priority: 2,
    ...
    if (!currentSettings.autoInactiveHandling && memberCount > 5) {
      recommendations.push({
        ...
        recommendedValue: true,
        reason: 'El manejo automático de inactivos mantiene el flujo de la sala',
        priority: 3,
    ...
    if (currentSettings.privacy === RoomPrivacy.PUBLIC && !currentSettings.requireApproval) {
      recommendations.push({
   ...
   */
  private calculateOptimizationScore(settings: AdvancedRoomSettings, room: any): number {
    let score = 0;
    const maxScore = 100;
    ...
    const memberCount = room.members?.length || 0;
    if (memberCount > 10 && settings.consensusThreshold !== ConsensusType.UNANIMOUS) score += 15;
    if (settings.votingTimeout && settings.votingTimeout >= 45) score += 10;
    if (settings.maxMembers && settings.maxMembers >= memberCount * 1.5) score += 15;
  ...
  private async verifyRoomAdmin(roomId: string, userId: string): Promise<void> {
    try {
      ...
      const isAdmin = room.creatorId === userId || 
                     room.members?.some(member => member.userId === userId && member.role === 'admin');
      
      if (!isAdmin) {
        throw new ForbiddenException('Solo los administradores pueden modificar las configuraciones');
      }
    } catch (error) {
      if (error instanceof NotFoundException) {
        throw new NotFoundException('Sala no encontrada');
  ...
  async migrateExistingRooms(): Promise<{ migrated: number; errors: number }> {
    this.logger.log('Iniciando migración de salas existentes a configuraciones avanzadas');
    ...
    try {
      ...
      this.logger.log(`Migración completada: ${migrated} salas migradas, ${errors} errores`);

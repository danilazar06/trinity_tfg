...
describe('RoomSettingsService Property Tests', () => {
  ...
  const advancedRoomSettingsArb = fc.record({
  ...
  const updateRoomSettingsDtoArb = fc.record({
    votingTimeout: fc.option(fc.integer({ min: 30, max: 300 })),
    sessionTimeout: fc.option(fc.integer({ min: 15, max: 480 })),
    ...
    if (settings.anonymousVoting === true && settings.showVotingProgress === true) {
      return false;
    }
    if (settings.consensusThreshold === ConsensusType.CUSTOM && !settings.customThreshold) {
      return false;
    }
    if (settings.contentInjectionEnabled === true && !settings.injectionFrequency) {
      return false;
    }
    return true;
  });
    ...
    it('should return valid settings for any room and user combination', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, mockSettings) => {
          ...
          expect(result.votingTimeout === null || typeof result.votingTimeout === 'number').toBe(true);
          expect(result.sessionTimeout === null || typeof result.sessionTimeout === 'number').toBe(true);
          expect(Object.values(ConsensusType)).toContain(result.consensusThreshold);
    ...
    it('should preserve valid settings when updating', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, currentSettings, updates) => {
          ...
          // Act
          const result = await service.updateRoomSettings(roomId, userId, updates);
          ...
          if (updates.consensusThreshold !== undefined) {
            expect(result.consensusThreshold).toBe(updates.consensusThreshold);
          } else {
            expect(result.consensusThreshold).toBe(currentSettings.consensusThreshold);
          }
    ...
    it('should reject invalid settings combinations', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, invalidSettings) => {
          ...
          // Act & Assert
          await expect(service.updateRoomSettings(roomId, userId, invalidSettings))
    ...
    it('should enforce proper access control for settings operations', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, adminUserId, regularUserId, updates) => {
          ...
          await expect(service.updateRoomSettings(roomId, adminUserId, updates))
            .resolves.toBeDefined();

          // Act & Assert - Regular user should be rejected
          if (regularUserId !== adminUserId) {
            await expect(service.updateRoomSettings(roomId, regularUserId, updates))
    ...
    it('should always return valid default settings', async () => {
      await fc.assert(fc.property(
        ...
        () => {
          ...
          expect(Object.values(ConsensusType)).toContain(defaultSettings.consensusThreshold);
          expect(Object.values(RoomPrivacy)).toContain(defaultSettings.privacy);
          ...
          expect(typeof defaultSettings.contentInjectionEnabled).toBe('boolean');
    ...
    it('should generate appropriate recommendations based on room context', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (room, currentSettings) => {
          ...
          // Act
          const result = await service.getSettingsRecommendations(room.id, userId);
      ...
      ), { numRuns: 50 }); // Menos iteraciones para operaciones complejas
    });
  });
    ...
    it('should maintain data integrity across save and load operations', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.record({
          votingTimeout: fc.option(fc.integer({ min: 30, max: 300 })),
          sessionTimeout: fc.option(fc.integer({ min: 15, max: 480 })),
          consensusThreshold: fc.option(fc.constantFrom(ConsensusType.UNANIMOUS, ConsensusType.MAJORITY, ConsensusType.SUPER_MAJORITY)), // Evitar CUSTOM
          privacy: fc.option(roomPrivacyArb),
          maxMembers: fc.option(fc.integer({ min: 2, max: 50 })),
          requireApproval: fc.option(fc.boolean()),
          allowGuestVoting: fc.option(fc.boolean()),
        ...
        async (roomId, userId, originalSettings) => {
          ...
          // Act - Load settings
          const loadedSettings = await service.getRoomSettings(roomId, userId);

          // Assert - Data integrity maintained
          if (originalSettings.votingTimeout !== undefined) {
            expect(loadedSettings.votingTimeout).toBe(originalSettings.votingTimeout);
          }
          if (originalSettings.sessionTimeout !== undefined) {
            expect(loadedSettings.sessionTimeout).toBe(originalSettings.sessionTimeout);
          }
          if (originalSettings.consensusThreshold !== undefined) {
            expect(loadedSettings.consensusThreshold).toBe(originalSettings.consensusThreshold);
          }
          ...
          if (originalSettings.requireApproval !== undefined) {
            expect(loadedSettings.requireApproval).toBe(originalSettings.requireApproval);
          }
          if (originalSettings.allowGuestVoting !== undefined) {
            expect(loadedSettings.allowGuestVoting).toBe(originalSettings.allowGuestVoting);
          }
          if (originalSettings.contentInjectionEnabled !== undefined) {
            expect(loadedSettings.contentInjectionEnabled).toBe(originalSettings.contentInjectionEnabled);
          }
          if (originalSettings.chatEnabled !== undefined) {
            expect(loadedSettings.chatEnabled).toBe(originalSettings.chatEnabled);
          }
        }
      ), { numRuns: 100 });
    });

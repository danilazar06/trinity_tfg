...
describe('RoomThemeService', () => {
  ...
  const updateThemeDtoArb = fc.record({
    name: fc.option(fc.string({ minLength: 3, maxLength: 50 }).filter(s => s.trim().length >= 3), { nil: undefined }),
  ...
  const themeCustomizationsArb = fc.record({
    colorOverrides: fc.option(fc.record({
        ...
        fc.asyncProperty(
          ...
          async (userId, createThemeDto) => {
            ...
            if (createThemeDto.backgroundImage) {
              expect(result.backgroundImage).toBe(createThemeDto.backgroundImage);
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (themeId, userId, originalTheme, updateDto) => {
            ...
            // Execute
            const result = await service.updateTheme(themeId, userId, updateDto);
            ...
            expect(result.updatedAt.getTime()).toBeGreaterThan(existingTheme.updatedAt.getTime());
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, userId, applyThemeDto) => {
            ...
            // Execute
            const result = await service.applyThemeToRoom(roomId, userId, applyThemeDto);
            ...
            if (applyThemeDto.customizations) {
              expect(result.customizations).toEqual(applyThemeDto.customizations);
            }
          }
        ),
        { numRuns: 50 }
        ...
        fc.asyncProperty(
          ...
          async (customizations) => {
            ...
            if (customizations.fontSize !== undefined) {
              expect(['small', 'medium', 'large']).toContain(customizations.fontSize);
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (themeId, userId, rateThemeDto) => {
            ...
            // Execute
            const result = await service.rateTheme(themeId, userId, rateThemeDto);
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should prevent duplicate theme names for same user', async () => {
      ...
      await expect(service.createTheme('user1', createThemeDto))
    ...
    it('should prevent system theme modification', async () => {
      ...
      await expect(service.updateTheme('dark-cinema', 'user1', updateDto))
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (createThemeDto) => {
            ...
            expect(Object.values(ThemeCategory)).toContain(createThemeDto.category);
            ...
            if (createThemeDto.backgroundImage) {
              expect(() => new URL(createThemeDto.backgroundImage!)).not.toThrow();
            }
            if (createThemeDto.icon) {
    ...
    it('should handle theme not found scenarios', async () => {
      ...
      await expect(service.getTheme('non-existent-theme'))
    ...
    it('should prevent unauthorized theme access', async () => {
      ...
      await expect(service.applyThemeToRoom('room1', 'other-user', applyThemeDto))
        .rejects.toThrow(ForbiddenException);
    });
  });
});
...
describe('RoomModerationService', () => {
  ...
  const permissionsArb = fc.array(permissionArb, { minLength: 1, maxLength: 10 }).map(perms => [...new Set(perms)]);
  
  const systemRoleArb = fc.constantFrom(...Object.values(SystemRole));
  const moderationActionTypeArb = fc.constantFrom(...Object.values(ModerationActionType));
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId) => {
            ...
            const ownerRole = systemRoles.find(role => role.id === SystemRole.OWNER);
            expect(ownerRole?.permissions).toEqual(Object.values(RoomPermission));

            // Verify guest has minimal permissions
            const guestRole = systemRoles.find(role => role.id === SystemRole.GUEST);
            expect(guestRole?.permissions).toContain(RoomPermission.VIEW_ROOM);
            expect(guestRole?.permissions).not.toContain(RoomPermission.DELETE_ROOM);
          }
        ),
        { numRuns: 50 }
      );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId) => {
            ...
            const ownerRole = systemRoles.find(role => role.id === SystemRole.OWNER);
            const otherRoles = systemRoles.filter(role => role.id !== SystemRole.OWNER);
            
            expect(ownerRole?.priority).toBeGreaterThan(Math.max(...otherRoles.map(r => r.priority)));

            // Verificar que Guest tiene la prioridad más baja
            const guestRole = systemRoles.find(role => role.id === SystemRole.GUEST);
            const nonGuestRoles = systemRoles.filter(role => role.id !== SystemRole.GUEST);
            
            expect(guestRole?.priority).toBeLessThan(Math.min(...nonGuestRoles.map(r => r.priority)));
          }
        ),
        { numRuns: 50 }
      );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, userId, createRoleDto) => {
            ...
            checkPermissionSpy.mockResolvedValue({ hasPermission: true, currentRoles: ['owner'] });

            try {
              const result = await service.createCustomRole(roomId, userId, createRoleDto);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, roleId, userId, originalRole, updateDto) => {
            ...
            try {
              const result = await service.updateCustomRole(roomId, roleId, userId, updateDto);
              ...
              expect(result.updatedAt.getTime()).toBeGreaterThan(existingRole.updatedAt.getTime());
            } finally {
              checkPermissionSpy.mockRestore();
            }
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, moderatorId, targetUserId, reason) => {
            ...
            // Bypass permission checks
            const checkPermissionSpy = jest.spyOn(service, 'checkPermission' as any);
            checkPermissionSpy.mockResolvedValue({ hasPermission: true, currentRoles: ['admin'] });

            const beforeAction = new Date();
            
            try {
              const result = await service.warnMember(roomId, { targetUserId, reason }, moderatorId);
              
              const afterAction = new Date();

              // Verify timestamps
              expect(result.createdAt).toBeInstanceOf(Date);
              expect(result.createdAt.getTime()).toBeGreaterThanOrEqual(beforeAction.getTime());
              expect(result.createdAt.getTime()).toBeLessThanOrEqual(afterAction.getTime());
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, moderatorId, targetUserId, reason, duration) => {
            ...
            try {
              const result = await service.muteMember(roomId, { targetUserId, reason, duration }, moderatorId);
              ...
              if (result.expiresAt) {
                const expectedExpiration = new Date(result.createdAt.getTime() + duration * 60 * 1000);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, moderatorId, targetUserId, reason, duration) => {
            ...
            try {
              const result = await service.banMember(roomId, { targetUserId, reason, duration }, moderatorId);

              // Verify ban type
              if (duration === 0) {
                expect(result.actionType).toBe(ModerationActionType.PERMANENT_BAN);
                expect(result.expiresAt).toBeUndefined();
              } else {
                expect(result.actionType).toBe(ModerationActionType.TEMPORARY_BAN);
                expect(result.expiresAt).toBeInstanceOf(Date);
              }
  ...
  describe('Edge cases and error handling', () => {
    it('should handle empty permission arrays', async () => {
      ...
      await expect(service.createCustomRole('room1', 'user1', createRoleDto))
        .rejects.toThrow();
    });
    ...
    it('should prevent system role modification', async () => {
      ...
      try {
        await expect(service.updateCustomRole('room1', SystemRole.OWNER, 'user1', updateDto))
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (createRoleDto) => {
            ...
            const hasValidPermissions = createRoleDto.permissions.every(p => validPermissions.includes(p));

...
import { CostOptimizationService, CostMetrics, CostOptimizationRecommendation, BudgetStatus } from './cost-optimization.service';
...
export class CostOptimizationController {
  ...
  @ApiOperation({ 
    ...
    description: 'Retorna métricas detalladas de costos de AWS incluyendo Lambda, DynamoDB y costos estimados'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      type: 'object',
      properties: {
        estimatedMonthlyCost: { type: 'number', example: 25.50 },
  ...
  @ApiOperation({ 
    ...
    description: 'Genera recomendaciones personalizadas para optimizar costos basadas en el uso actual'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      ...
      items: {
        ...
        properties: {
          ...
          description: { type: 'string', example: 'Se detectaron 15000 invocaciones en las últimas 24h' },
          potentialSavings: { type: 'number', example: 12.50 },
          actionRequired: { type: 'string', example: 'Implementar caché en endpoints frecuentes' }
        }
      }
    }
  })
  ...
  @ApiResponse({ 
    ...
    schema: {
      ...
      properties: {
        ...
        budgetLimit: { type: 'number', example: 50.00 },
        actualSpend: { type: 'number', example: 23.45 },
        forecastedSpend: { type: 'number', example: 45.20 },
  ...
  @ApiOperation({ 
    ...
    description: 'Ejecuta optimizaciones automáticas seguras como limpieza de logs y configuración de concurrencia'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      ...
      properties: {
        ...
        estimatedSavings: { type: 'number', example: 8.50 }
      }
    }
  })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  ...
  async applyAutomaticOptimizations(): Promise<{
    ...
    const appliedOptimizations = await this.costOptimizationService.applyAutomaticOptimizations();
  ...
  @ApiOperation({ 
    ...
    description: 'Verifica que todos los servicios de AWS estén disponibles para el monitoreo de costos'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiResponse({ status: 503, description: 'Algunos servicios no están disponibles' })
  ...
  @ApiOperation({ 
    ...
    description: 'Retorna la URL del dashboard de CloudWatch para monitoreo visual de costos'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      ...
      properties: {
        dashboardUrl: { 
          type: 'string', 
          example: 'https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=trinity-cost-monitoring-dev' 
        },

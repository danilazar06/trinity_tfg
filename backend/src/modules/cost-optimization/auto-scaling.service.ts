...
export class AutoScalingService {
  ...
  async hourlyOptimizationCheck() {
    try {
      this.logger.log('🔄 Iniciando verificación automática de optimización...');

      // Obtener métricas actuales
      const metrics = await this.costOptimizationService.getCurrentCostMetrics();
      ...
      if (needsOptimization) {
        this.logger.warn('⚠️ Se detectaron métricas que requieren optimización automática');
        
        // Aplicar optimizaciones automáticas
        const optimizations = await this.costOptimizationService.applyAutomaticOptimizations();
        
        this.logger.log(`✅ Aplicadas ${optimizations.length} optimizaciones automáticas`);
        optimizations.forEach(opt => this.logger.log(`  - ${opt}`));
      } else {
        this.logger.log('✅ Métricas dentro de rangos normales, no se requiere optimización');
      }

    } catch (error) {
      this.logger.error('❌ Error en verificación automática de optimización:', error);
    }
  }
  ...
  async dailyCostReport() {
    try {
      ...
      this.logger.log(`💰 Costo estimado mensual: $${metrics.estimatedMonthlyCost}`);
      this.logger.log(`🚀 Invocaciones Lambda (24h): ${metrics.lambdaInvocations}`);
      this.logger.log(`📖 DynamoDB RCU (24h): ${metrics.dynamoReadUnits}`);
      this.logger.log(`✏️ DynamoDB WCU (24h): ${metrics.dynamoWriteUnits}`);

      if (budgetStatus) {
        this.logger.log(`📈 Presupuesto usado: ${budgetStatus.percentageUsed.toFixed(1)}% ($${budgetStatus.actualSpend}/$${budgetStatus.budgetLimit})`);
        this.logger.log(`📅 Días restantes del mes: ${budgetStatus.daysRemaining}`);
      }

      if (recommendations.length > 0) {
        this.logger.log(`🎯 Recomendaciones de optimización: ${recommendations.length}`);
        recommendations.slice(0, 3).forEach(rec => {
          this.logger.log(`  - [${rec.severity.toUpperCase()}] ${rec.title} (Ahorro: $${rec.potentialSavings.toFixed(2)})`);
        });
      }
  ...
  async weeklyMaintenance() {
    try {
      ...
      this.logger.log(`🧹 Mantenimiento semanal completado: ${optimizations.length} optimizaciones aplicadas`);
      optimizations.forEach(opt => this.logger.log(`  - ${opt}`));
  ...
  private shouldApplyOptimizations(metrics: any): boolean {
    ...
    const highDynamoUsage = metrics.dynamoReadUnits > 500 || metrics.dynamoWriteUnits > 250;
  ...
  private checkCriticalAlerts(metrics: any, budgetStatus: any): string[] {
    ...
    if (budgetStatus && budgetStatus.percentageUsed > 90) {
      alerts.push(`Presupuesto al ${budgetStatus.percentageUsed.toFixed(1)}% - CRÍTICO`);
    }

    // Alerta de invocaciones Lambda excesivas
    if (metrics.lambdaInvocations > 10000) {
      alerts.push(`Invocaciones Lambda excesivas: ${metrics.lambdaInvocations} en las últimas 24h`);
    }

    // Alerta de costo mensual alto
    if (metrics.estimatedMonthlyCost > 200) {
      alerts.push(`Costo mensual estimado muy alto: $${metrics.estimatedMonthlyCost}`);
    }

    // Alerta de uso excesivo de DynamoDB
    if (metrics.dynamoReadUnits > 2000 || metrics.dynamoWriteUnits > 1000) {
      alerts.push(`Uso excesivo de DynamoDB: ${metrics.dynamoReadUnits} RCU, ${metrics.dynamoWriteUnits} WCU`);
    }

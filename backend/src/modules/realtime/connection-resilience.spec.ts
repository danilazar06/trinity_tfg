...
const createMockGraphQLClient = (shouldFail: boolean = false, failureCount: number = 0) => {
  let callCount = 0;
  return {
    ...
    it('should handle GraphQL client failures gracefully without throwing errors', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, mediaId, voteType, progress) => {
          ...
          await expect(appSyncPublisher.publishVoteUpdate(roomId, voteData)).resolves.not.toThrow();
        ...
        async (failurePattern, roomId) => {
          ...
          const mockIntermittentClient = {
            request: jest.fn().mockImplementation(() => {
              ...
              if (shouldFail) {
                return Promise.reject(new Error(`Intermittent failure ${callIndex}`));
              }
              return Promise.resolve({ success: true });
            })
          };
          ...
          expect(mockIntermittentClient.request).toHaveBeenCalledTimes(failurePattern.length);
        }
      ), { numRuns: 30 });
    });
    ...
    it('should handle various error types gracefully', async () => {
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (errorIndex, roomId) => {
          ...
          await expect(appSyncPublisher.publishMatchFound(roomId, matchData)).resolves.not.toThrow();
    ...
    it('should maintain functional equivalence through the compatibility service even with failures', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, status, queueLength, activeMembers) => {
          ...
          await expect(realtimeService.notifyRoomStateChange(roomId, stateData)).resolves.not.toThrow();
        }
      ), { numRuns: 50 });
    });
        ...
        async (concurrentOps, roomId) => {
          ...
          const operations = Array.from({ length: concurrentOps }, (_, i) => {
            const voteData = {
              ...
              mediaId: `media-${i}`,
              voteType: 'like' as const,
              progress: { totalVotes: i + 1, requiredVotes: 10, percentage: (i + 1) * 10 }
            };
          ...
          // Verify all attempts were made
          expect(mockRandomFailClient.request).toHaveBeenCalledTimes(concurrentOps);
        }
      ), { numRuns: 30 });
    });
  });

...
describe('AppSyncPublisher - Property Tests', () => {
    ...
    it('should successfully publish vote events for any valid vote data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, mediaId, voteType, progress) => {
          ...
          await expect(service.publishVoteUpdate(roomId, voteData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish match events for any valid match data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, mediaId, mediaTitle, participants, matchType) => {
          ...
          // Should not throw an error
          await expect(service.publishMatchFound(roomId, matchData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish room state changes for any valid state data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, status, currentMediaId, queueLength, activeMembers) => {
          ...
          // Should not throw an error
          await expect(service.publishRoomStateChange(roomId, stateData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish member status changes for any valid member data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId, status, lastActivity) => {
          ...
          // Should not throw an error
          await expect(service.publishMemberStatusChange(roomId, memberData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish role assignment events for any valid role data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, targetUserId, roleId, roleName, assignedBy, action) => {
          ...
          // Should not throw an error
          await expect(service.publishRoleAssignment(roomId, roleData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish moderation action events for any valid moderation data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, targetUserId, moderatorId, actionType, reason, duration, expiresAt) => {
          ...
          // Should not throw an error
          await expect(service.publishModerationAction(roomId, moderationData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish schedule events for any valid schedule data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, scheduleId, title, action, startTime, endTime, message) => {
          ...
          // Should not throw an error
          await expect(service.publishScheduleEvent(roomId, scheduleData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish theme change events for any valid theme data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, themeId, themeName, action, appliedBy, customizations) => {
          ...
          // Should not throw an error
          await expect(service.publishThemeChange(roomId, themeData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish room settings change events for any valid settings data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, settingKey, oldValue, newValue, changedBy, category) => {
          ...
          // Should not throw an error
          await expect(service.publishRoomSettingsChange(roomId, settingsData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish chat message events for any valid chat data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, messageId, userId, username, type, message, data) => {
          ...
          // Should not throw an error
          await expect(service.publishChatMessage(roomId, chatData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
    ...
    it('should successfully publish content suggestion events for any valid suggestion data', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, suggestionId, userId, username, type, suggestion, vote, comment) => {
          ...
          // Should not throw an error
          await expect(service.publishContentSuggestion(roomId, suggestionData)).resolves.not.toThrow();
        }
      ), { numRuns: 100 });
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle GraphQL errors gracefully without throwing', async () => {
      ...
      await expect(service.publishVoteUpdate('test-room', voteData)).resolves.not.toThrow();
    });
  });
});
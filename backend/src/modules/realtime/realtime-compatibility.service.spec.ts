...
describe('RealtimeCompatibilityService - Property Tests', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<RealtimeCompatibilityService>(RealtimeCompatibilityService);
  ...
  describe('Property 3: Service Integration Equivalence', () => {
    ...
    it('should maintain functional equivalence for match notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, mediaId, mediaTitle, participants, matchType) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishMatchFound).toHaveBeenCalledWith(roomId, matchData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for room state change notifications', async () => {
      await fc.assert(fc.asyncProperty(
        fc.string({ minLength: 1, maxLength: 50 }), // roomId
        fc.oneof(fc.constant('active'), fc.constant('paused'), fc.constant('finished')), // status
        ...
        async (roomId, status, currentMediaId, queueLength, activeMembers) => {
          const stateData = {
            status,
            currentMediaId,
            queueLength,
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishRoomStateChange).toHaveBeenCalledWith(roomId, stateData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for member status change notifications', async () => {
      await fc.assert(fc.asyncProperty(
        fc.string({ minLength: 1, maxLength: 50 }), // roomId
        fc.string({ minLength: 1, maxLength: 50 }), // userId
        fc.oneof(fc.constant('active'), fc.constant('inactive'), fc.constant('left')), // status
        ...
        async (roomId, userId, status, lastActivity) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishMemberStatusChange).toHaveBeenCalledWith(roomId, memberData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for role assignment notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, targetUserId, roleId, roleName, assignedBy, action) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishRoleAssignment).toHaveBeenCalledWith(roomId, roleData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for moderation action notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, targetUserId, moderatorId, actionType, reason, duration, expiresAt) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishModerationAction).toHaveBeenCalledWith(roomId, moderationData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for schedule event notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.oneof(fc.constant('created'), fc.constant('updated'), fc.constant('cancelled'), fc.constant('reminder')), // action
        fc.string(), // startTime
        fc.string(), // endTime
        fc.option(fc.string({ maxLength: 500 })), // message
        async (roomId, scheduleId, title, action, startTime, endTime, message) => {
          const scheduleData = {
            scheduleId,
            title,
            action,
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishScheduleEvent).toHaveBeenCalledWith(roomId, scheduleData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for theme change notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.oneof(fc.constant('applied'), fc.constant('removed'), fc.constant('updated')), // action
        ...
        async (roomId, themeId, themeName, action, appliedBy, customizations) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishThemeChange).toHaveBeenCalledWith(roomId, themeData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for room settings change notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.oneof(fc.constant('privacy'), fc.constant('consensus'), fc.constant('capacity'), fc.constant('timeout'), fc.constant('other')), // category
        async (roomId, settingKey, oldValue, newValue, changedBy, category) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishRoomSettingsChange).toHaveBeenCalledWith(roomId, settingsData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for chat message notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.oneof(fc.constant('message'), fc.constant('edit'), fc.constant('delete'), fc.constant('reaction')), // type
        ...
        async (roomId, messageId, userId, username, type, message, data) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishChatMessage).toHaveBeenCalledWith(roomId, chatData);
        }
      ), { numRuns: 100 });
    });
    ...
    it('should maintain functional equivalence for content suggestion notifications', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        fc.string({ minLength: 1, maxLength: 50 }), // userId
        fc.string({ minLength: 1, maxLength: 50 }), // username
        fc.oneof(fc.constant('created'), fc.constant('voted'), fc.constant('commented'), fc.constant('approved'), fc.constant('rejected'), fc.constant('implemented')), // type
        fc.option(fc.object()), // suggestion
        fc.option(fc.object()), // vote
        fc.option(fc.object()), // comment
        async (roomId, suggestionId, userId, username, type, suggestion, vote, comment) => {
          ...
          // Verify that the AppSync publisher was called with the same data
          expect(mockAppSyncPublisher.publishContentSuggestion).toHaveBeenCalledWith(roomId, suggestionData);
        }
      ), { numRuns: 100 });
    });
  });
  ...
  describe('Legacy Method Compatibility', () => {
    it('should handle queue progress notifications by converting to room state changes', async () => {
      ...
      expect(mockAppSyncPublisher.publishRoomStateChange).toHaveBeenCalledWith(roomId, {
        status: 'active',
    ...
    it('should handle unsupported async methods gracefully', async () => {
      // These methods should not throw errors but should log warnings
      await expect(service.broadcastSystemMessage('test message')).resolves.not.toThrow();
      await expect(service.sendPrivateMessage('user-123', { test: 'message' })).resolves.not.toThrow();
      
      const result = await service.sendPrivateMessage('user-123', { test: 'message' });

  ...
  describe('Property 6: Subscription Filtering Accuracy', () => {
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, isMember) => {
          ...
          mockDynamoClient.send.mockResolvedValue({
            Item: isMember ? { roomId, userId, role: 'member', joinedAt: new Date().toISOString() } : undefined
          });
    ...
    it('should handle various room membership scenarios consistently', async () => {
      const membershipScenarios = [
        { role: 'admin', permissions: ['admin', 'moderator', 'member'], shouldHaveAccess: true },
        { role: 'moderator', permissions: ['moderator', 'member'], shouldHaveAccess: true },
    ...
    it('should validate filter criteria format for any valid room', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId) => {
          ...
          mockDynamoClient.send.mockResolvedValue({
            Item: { roomId, userId, role: 'member', joinedAt: new Date().toISOString() }
          });
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, isMember, numCalls) => {
          ...
          const results = await Promise.all(
            Array.from({ length: numCalls }, () => roomEventFilter(mockEvent as any, {} as any))
          );
    ...
    it('should handle concurrent filtering requests without interference', async () => {
      await fc.assert(fc.asyncProperty(
        fc.array(fc.string({ minLength: 1, maxLength: 50 }), { minLength: 2, maxLength: 10 }), // userIds
        fc.string({ minLength: 1, maxLength: 50 }), // roomId
        fc.array(fc.boolean(), { minLength: 2, maxLength: 10 }), // membership statuses
        async (userIds, roomId, membershipStatuses) => {
          // Ensure arrays have same length
          const minLength = Math.min(userIds.length, membershipStatuses.length);
          ...
          mockDynamoClient.send.mockImplementation((command) => {
            const userId = testUserIds.find(id => command.input?.Key?.userId === id);
          ...
          results.forEach((result, index) => {
            const expectedResult = testMemberships[index] ? { roomId: { eq: roomId } } : null;
            expect(result).toEqual(expectedResult);
          });
        }
      ), { numRuns: 30 });

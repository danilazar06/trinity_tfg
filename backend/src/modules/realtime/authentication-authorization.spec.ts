  ...
  describe('Property 2: Authentication and Authorization Consistency', () => {
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, hasAccess) => {
          ...
          if (hasAccess) {
            ...
            await expect(publishVoteEvent(mockEvent as any, {} as any)).resolves.toBeDefined();
          } else {
            // Should throw unauthorized error when user doesn't have access
            await expect(publishVoteEvent(mockEvent as any, {} as any)).rejects.toThrow('Unauthorized');
          }
    ...
    it('should enforce role-based permissions for administrative actions', async () => {
      await fc.assert(fc.asyncProperty(
        fc.string({ minLength: 1, maxLength: 50 }), // userId
        fc.string({ minLength: 1, maxLength: 50 }), // roomId
        fc.oneof(fc.constant('admin'), fc.constant('moderator'), fc.constant('member')), // userRole
        async (userId, roomId, userRole) => {
          ...
          mockDynamoClient.send
            ...
            .mockResolvedValueOnce({
              Item: { 
                roomId, 
                userId, 
                permissions: userRole === 'admin' ? ['admin', 'moderator', 'member'] :
          ...
          if (userRole === 'admin' || userRole === 'moderator') {
            ...
            await expect(publishRoleEvent(mockRoleEvent as any, {} as any)).resolves.toBeDefined();
          } else {
            // Should fail for regular members
            await expect(publishRoleEvent(mockRoleEvent as any, {} as any)).rejects.toThrow('Insufficient permissions');
          }
        }
      ), { numRuns: 50 });
    });
    ...
    it('should enforce moderation permissions for moderation actions', async () => {
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, userRole, targetUserId, actionType, reason) => {
          ...
          if (userRole === 'admin' || userRole === 'moderator') {
            // Should succeed for admin/moderator
            await expect(publishModerationEvent(mockModerationEvent as any, {} as any)).resolves.toBeDefined();
          } else {
            // Should fail for regular members
            await expect(publishModerationEvent(mockModerationEvent as any, {} as any)).rejects.toThrow('Insufficient permissions');
          }
        }
      ), { numRuns: 50 });
    });
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, userRole, settingKey, newValue) => {
          ...
          if (userRole === 'admin') {
            ...
            await expect(publishSettingsEvent(mockSettingsEvent as any, {} as any)).resolves.toBeDefined();
          } else {
            // Should fail for moderator and member
            await expect(publishSettingsEvent(mockSettingsEvent as any, {} as any)).rejects.toThrow('Only room admins can change settings');
          }
        }
      ), { numRuns: 50 });
    });
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (roomId, userId) => {
          ...
          if (!userId) {
            ...
            await expect(publishVoteEvent(mockEvent as any, {} as any)).rejects.toThrow();
          } else {
            // Valid userId should proceed to room access check
            mockDynamoClient.send.mockResolvedValue({ Item: { roomId, userId } });
            await expect(publishVoteEvent(mockEvent as any, {} as any)).resolves.toBeDefined();
          }
        }
      ), { numRuns: 50 });
      ...
      await fc.assert(fc.asyncProperty(
        ...
        async (userId, roomId, eventTypeIndex, userRole) => {
          ...
          const hasRequiredPermission = 
            ...
            (eventType.requiresPermission === 'moderator' && (userRole === 'admin' || userRole === 'moderator')) ||
            (eventType.requiresPermission === 'admin' && userRole === 'admin');
          ...
          switch (eventType.name) {
            case 'vote':
              mockEvent = {
                arguments: { roomId, voteData: { userId, mediaId: 'test', voteType: 'LIKE', progress: {} } },
                identity: { sub: userId }
              };
            ...
            case 'role':
              mockEvent = {
                arguments: { roomId, roleData: { targetUserId: 'target', roleId: 'role', roleName: 'test', action: 'ASSIGNED' } },
                identity: { sub: userId }
              };
            ...
            case 'moderation':
              mockEvent = {
                arguments: { roomId, moderationData: { targetUserId: 'target', actionType: 'warn', reason: 'test' } },
                identity: { sub: userId }
              };
            ...
            case 'settings':
              mockEvent = {
                arguments: { roomId, settingsData: { settingKey: 'test', oldValue: 'old', newValue: 'new', category: 'PRIVACY' } },
                identity: { sub: userId }
              };
          ...
          if (hasRequiredPermission) {
            await expect(publishFunction(mockEvent as any, {} as any)).resolves.toBeDefined();
          } else {
            await expect(publishFunction(mockEvent as any, {} as any)).rejects.toThrow();
          }
        }
      ), { numRuns: 100 });
    });

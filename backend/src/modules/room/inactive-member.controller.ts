...
import { InactiveMemberService, InactivityConfig } from './inactive-member.service';
...
export class InactiveMemberController {
  ...
  @ApiOperation({
    ...
    description: 'Obtiene un reporte detallado del estado de actividad de todos los miembros de la sala'
  })
  @ApiResponse({
    status: 200,
  ...
  async getActivityReport(
    ...
    this.logger.log(`Getting activity report for room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({
    ...
    description: 'Obtiene la lista de miembros que deben ser incluidos en el cálculo de consenso'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Obtiene el estado detallado de actividad de un miembro específico'
  })
  @ApiResponse({
    status: 200,
  ...
  async getMemberActivityStatus(
    ...
    const status = await this.inactiveMemberService.getMemberActivityStatus(roomId, userId);
  ...
  @ApiOperation({
    ...
    description: 'Ejecuta manualmente la limpieza de miembros inactivos en la sala'
  })
  @ApiResponse({
    status: 200,
  ...
  async cleanupInactiveMembers(
    ...
    this.logger.log(`Manual cleanup requested for room ${roomId} by user ${req.user.sub}`);
  ...
  async reactivateMember(
    ...
    this.logger.log(`Reactivating member ${userId} in room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({
    ...
    description: 'Actualiza la última actividad de un miembro (usado internamente por otras acciones)'
  })
  @ApiResponse({
    status: 200,
...
export class InactivityConfigController {
  ...
  @ApiOperation({
    ...
    description: 'Obtiene la configuración actual de manejo de miembros inactivos'
  })
  @ApiResponse({
    status: 200,

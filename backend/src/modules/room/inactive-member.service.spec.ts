...
import { InactiveMemberService, ActivityLevel, InactivityConfig } from './inactive-member.service';
    ...
    it('should exclude inactive members from voting calculations', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(fc.integer({ min: 0, max: 120 }), { minLength: 3, maxLength: 8 }), // minutesInactive
          async (roomId, userIds, minutesInactive) => {
            ...
            const mockMembers: Member[] = testUserIds.map((userId, index) => {
              ...
              const lastActivityAt = new Date(now.getTime() - minutesAgo * 60 * 1000);
            ...
            const activeMembersForVoting = await service.getActiveMembersForVoting(roomId);

            // Assert: Verify inactive member exclusion logic
            const expectedActiveMembers = mockMembers.filter((member, index) => {
              const minutesAgo = testMinutesInactive[index];
              // Members inactive for more than 30 minutes should be excluded
              return minutesAgo < 30;
            });

            expect(activeMembersForVoting).toHaveLength(expectedActiveMembers.length);

            // Verify that all returned members are actually active
            activeMembersForVoting.forEach(activeMember => {
              const originalMember = mockMembers.find(m => m.userId === activeMember.userId);
            ...
            const excludedUserIds = testUserIds.filter(userId => 
              !activeMembersForVoting.some(active => active.userId === userId)
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, userId, minutesInactive) => {
            ...
            const now = new Date();
            const lastActivityAt = new Date(now.getTime() - minutesInactive * 60 * 1000);
            ...
            const activityStatus = await service.getMemberActivityStatus(roomId, userId);
            ...
            if (minutesInactive >= 60) {
              expect(activityStatus!.activityLevel).toBe(ActivityLevel.EXCLUDED);
              ...
              expect(activityStatus!.activityLevel).toBe(ActivityLevel.INACTIVE);
              expect(activityStatus!.shouldExcludeFromVoting).toBe(true);
              expect(activityStatus!.status).toBe(MemberStatus.INACTIVE);
    ...
    it('should maintain consistency in activity reporting across multiple checks', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(fc.integer({ min: 0, max: 90 }), { minLength: 2, maxLength: 6 }), // minutesInactive
          async (roomId, userIds, minutesInactive) => {
            // Reset mocks for this iteration
            jest.clearAllMocks();
            ...
            const totalCounted = report1.activeMembers + report1.warningMembers + 
                               report1.inactiveMembers + report1.excludedMembers;
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (roomId, userId, initialMinutesInactive) => {
            ...
            const now = new Date();
            const lastActivityAt = new Date(now.getTime() - initialMinutesInactive * 60 * 1000);
            ...
            expect(memberService.getMember).toHaveBeenCalledWith(roomId, userId);
            expect(memberService.updateMemberActivity).toHaveBeenCalledWith(roomId, userId);
            
            // Verify it was called at least once (could be more due to internal logic)
            const callCount = memberService.updateMemberActivity.mock.calls.length;
    ...
    it('should calculate room activity statistics correctly', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.array(fc.integer({ min: 0, max: 180 }), { minLength: 1, maxLength: 10 }), // minutesInactive
          async (roomId, userIds, minutesInactive) => {
            // Reset mocks for this iteration
            jest.clearAllMocks();
            ...
            const activeMembers = testMinutesInactive.filter(minutes => minutes < 15).length;
            const inactiveMembers = testMinutesInactive.filter(minutes => minutes >= 15).length;

            expect(stats.activeMembers).toBe(activeMembers);
            expect(stats.inactiveMembers).toBe(inactiveMembers);

            // Verify total adds up
            expect(stats.activeMembers + stats.inactiveMembers).toBe(stats.totalMembers);

            // Verify most inactive member calculation
            if (inactiveMembers > 0) {
              const maxInactiveMinutes = Math.max(...testMinutesInactive.filter(m => m >= 15));
              expect(stats.mostInactiveMember).toBeDefined();
              expect(stats.mostInactiveMember!.minutesInactive).toBe(maxInactiveMinutes);
            } else {
            ...
            if (inactiveMembers > 0) {
              const inactiveMinutesArray = testMinutesInactive.filter(m => m >= 15);
              const expectedAverage = Math.round(
                inactiveMinutesArray.reduce((sum, m) => sum + m, 0) / inactiveMinutesArray.length
              );

...
export class ShuffleSyncController {
  ...
  @ApiOperation({ 
    ...
    description: 'Obtiene contenido de TMDB basado en filtros de la sala y genera listas desordenadas para todos los miembros'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async generateMasterListAndShuffledLists(
    ...
    this.logger.log(`Generating master list and shuffled lists for room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Regenera las listas desordenadas manteniendo la Lista Maestra existente'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async regenerateShuffledLists(
    ...
    this.logger.log(`Regenerating shuffled lists for room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Añade nuevo contenido a la Lista Maestra y actualiza las listas desordenadas'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async injectNewContent(
    ...
    this.logger.log(`Injecting content for room ${roomId} by user ${req.user.sub}`);
    
    return this.shuffleSyncService.injectNewContent(roomId, injectContentDto.mediaIds);
  }

  @Get('verify')
  @ApiOperation({ 
    summary: 'Verificar Consistencia Shuffle & Sync',
    description: 'Verifica que todas las listas desordenadas contengan los mismos elementos que la Lista Maestra'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async verifyShuffleSyncConsistency(
    ...
    this.logger.log(`Verifying shuffle sync consistency for room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Obtiene estadísticas del estado actual del mecanismo Shuffle & Sync'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async getShuffleSyncStats(
    ...
    this.logger.log(`Getting shuffle sync stats for room ${roomId} by user ${req.user.sub}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Resetea todas las listas de la sala (Lista Maestra y listas desordenadas)'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async resetRoomLists(
    ...
    this.logger.log(`Resetting room lists for room ${roomId} by user ${req.user.sub}`);

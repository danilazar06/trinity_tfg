...
// src/handlers/movie.ts
var movie_exports = {};
__export(movie_exports, {
  handler: () => handler
});
...
function logCacheMetric(cacheType, hit, key) {
  ...
  if (key) {
    console.log(`\u{1F4BE} CACHE_${hit ? "HIT" : "MISS"}: ${cacheType} - ${key}`);
  }
}
...
var CircuitBreaker = class {
  ...
  async execute(operation) {
    ...
    try {
      ...
      this.onSuccess();
      timer.finish(true, void 0, { state: this.state.state });
      return result;
    } catch (error) {
      this.onFailure(now);
      ...
      logError(`CircuitBreaker_${this.config.serviceName}`, error, {
    ...
    switch (this.state.state) {
      ...
      case "OPEN" /* OPEN */:
        if (now >= this.state.nextAttempt) {
          this.state.state = "HALF_OPEN" /* HALF_OPEN */;
          this.state.successCount = 0;
          console.log(`\u{1F504} Circuit breaker for ${this.config.serviceName} transitioning to HALF_OPEN`);
        }
        break;
    ...
    switch (this.state.state) {
      ...
      case "HALF_OPEN" /* HALF_OPEN */:
        ...
        if (this.state.successCount >= this.config.successThreshold) {
          this.state.state = "CLOSED" /* CLOSED */;
          this.state.failureCount = 0;
          this.state.successCount = 0;
    ...
    switch (this.state.state) {
      case "CLOSED" /* CLOSED */:
        ...
        if (this.state.failureCount >= this.config.failureThreshold) {
          this.state.state = "OPEN" /* OPEN */;
          this.state.nextAttempt = now + this.config.resetTimeout;
          console.log(`\u{1F6A8} Circuit breaker for ${this.config.serviceName} OPENED - too many failures (${this.state.failureCount})`);
        }
        break;
      case "HALF_OPEN" /* HALF_OPEN */:
        this.state.state = "OPEN" /* OPEN */;
        this.state.nextAttempt = now + this.config.resetTimeout;
        this.state.successCount = 0;
        console.log(`\u{1F6A8} Circuit breaker for ${this.config.serviceName} back to OPEN - half-open test failed`);
        break;
    }
    if (previousState !== this.state.state) {
      this.logCurrentState();
  ...
  forceOpen() {
    ...
    console.log(`\u{1F527} Circuit breaker for ${this.config.serviceName} manually OPENED`);
    this.logCurrentState();
  }
  forceClose() {
    this.state.state = "CLOSED" /* CLOSED */;
    this.state.failureCount = 0;
    this.state.successCount = 0;
    console.log(`\u{1F527} Circuit breaker for ${this.config.serviceName} manually CLOSED`);
    this.logCurrentState();
  }
};
var tmdbCircuitBreaker = new CircuitBreaker({
...
var handler = async (event) => {
  console.log("\u{1F3AC} Movie Handler:", JSON.stringify(event, null, 2));
...
async function getMovies(genre) {
  ...
  try {
    ...
    if (cachedMovies && cachedMovies.length > 0) {
      ...
      console.log(`\u{1F4BE} Pel\xEDculas obtenidas desde cache: ${cachedMovies.length}`);
      timer.finish(true, void 0, { source: "cache", count: cachedMovies.length });
      return cachedMovies;
    }
    logCacheMetric("MOVIES", false, cacheKey);
    console.log("\u{1F310} Obteniendo pel\xEDculas desde TMDB API con Circuit Breaker...");
    const moviesFromAPI = await tmdbCircuitBreaker.execute(() => fetchMoviesFromTMDB(genre));
    await cacheMovies(cacheKey, moviesFromAPI);
    console.log(`\u2705 Pel\xEDculas obtenidas desde API: ${moviesFromAPI.length}`);
    timer.finish(true, void 0, { source: "api", count: moviesFromAPI.length });
    ...
    if (fallbackMovies && fallbackMovies.length > 0) {
      console.log(`\u{1F504} Usando cache expirado como fallback: ${fallbackMovies.length}`);
      timer.finish(true, void 0, { source: "expired_cache", count: fallbackMovies.length });
      return fallbackMovies;
    }
    console.log("\u{1F3AD} Usando pel\xEDculas por defecto - Circuit Breaker protegi\xF3 el sistema");
    const defaultMovies = getDefaultMovies();
    timer.finish(true, void 0, { source: "default", count: defaultMovies.length });
...
async function getCachedMovies(cacheKey, allowExpired = false) {
  try {
    ...
    if (!allowExpired && cached.ttl < Math.floor(Date.now() / 1e3)) {
...
async function cacheMovies(cacheKey, movies) {
  try {
    ...
    console.log(`\u{1F4BE} Pel\xEDculas cacheadas: ${cacheKey}`);
  } catch (error) {
    console.warn("\u26A0\uFE0F Error cacheando pel\xEDculas:", error);
  }
}
async function fetchMoviesFromTMDB(genre) {
  ...
  if (!data.results || !Array.isArray(data.results)) {
    throw new Error("Formato de respuesta TMDB inv\xE1lido");
  }
  return data.results.slice(0, 20).map((movie) => ({
    id: movie.id.toString(),
    title: movie.title || movie.original_title || "T\xEDtulo no disponible",
    poster: movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : "https://via.placeholder.com/500x750?text=Sin+Poster",
    overview: movie.overview || "Descripci\xF3n no disponible"
  }));
}
function getGenreId(genreName) {
  ...
  return genreMap[genreName.toLowerCase()] || "28";
}
function getDefaultMovies() {
  return [
    ...
    },
    {
      ...
      overview: "Historias entrelazadas de crimen en Los \xC1ngeles."
    },
    {
      id: "default_3",
      title: "El Se\xF1or de los Anillos",
      poster: "https://via.placeholder.com/500x750?text=LOTR",
      overview: "Una \xE9pica aventura de fantas\xEDa en la Tierra Media."
    },
    {
      id: "default_4",
      title: "Forrest Gump",

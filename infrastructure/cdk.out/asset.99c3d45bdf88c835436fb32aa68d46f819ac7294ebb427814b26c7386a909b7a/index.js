...
// asset-input/src/handlers/ai.ts
var ai_exports = {};
__export(ai_exports, {
  handler: () => handler
});
...
var handler = async (event) => {
  console.log("\u{1F916} AI Handler:", JSON.stringify(event, null, 2));
...
async function getChatRecommendations(userText) {
  console.log(`\u{1F9E0} Analizando texto del usuario: "${userText}"`);
  try {
...
function buildContextualPrompt(userText) {
  const systemPrompt = `Eres un experto en cine y entretenimiento. Tu trabajo es analizar el sentimiento y contexto emocional del usuario para recomendar g\xE9neros cinematogr\xE1ficos apropiados.

Instrucciones:
- Analiza el texto del usuario para identificar su estado emocional
- Recomienda exactamente 3 g\xE9neros de pel\xEDculas que se adapten a su estado de \xE1nimo
- Responde SOLO con los g\xE9neros separados por comas, sin explicaciones adicionales
- G\xE9neros disponibles: acci\xF3n, aventura, animaci\xF3n, comedia, crimen, documental, drama, familia, fantas\xEDa, historia, terror, m\xFAsica, misterio, romance, ciencia ficci\xF3n, thriller, guerra, western

Ejemplos:
Usuario: "Estoy muy triste y necesito algo que me anime"
Respuesta: comedia, animaci\xF3n, familia

Usuario: "Quiero algo emocionante y lleno de adrenalina"
Respuesta: acci\xF3n, thriller, aventura

Usuario: "Me siento rom\xE1ntico esta noche"
...
async function callSalamandraAPI(prompt) {
  ...
  console.log(`\u{1F916} Respuesta de Salamandra: "${generatedText}"`);
  return generatedText;
}
function parseRecommendations(aiResponse) {
  try {
    const cleanResponse = aiResponse.toLowerCase().replace(/[^\w\s,áéíóúñ]/g, "").trim();
    const genres = cleanResponse.split(",").map((genre) => genre.trim()).filter((genre) => genre.length > 0).slice(0, 3);
    const validGenres = validateGenres(genres);
    if (validGenres.length === 0) {
      throw new Error("No se encontraron g\xE9neros v\xE1lidos en la respuesta");
    }
    return validGenres;
  } catch (error) {
...
function getFallbackRecommendations(userText) {
  ...
  if (text.includes("risa") || text.includes("divertido") || text.includes("gracioso")) {
    return ["comedia", "animaci\xF3n", "aventura"];
  }
  if (text.includes("pensar") || text.includes("reflexionar") || text.includes("profundo")) {
    return ["drama", "ciencia ficci\xF3n", "documental"];
  }
  return ["drama", "comedia", "acci\xF3n"];
}

...
/**
 * Cognito Authentication Test with Admin Confirmation
 * Tests user registration and login with AWS Cognito using admin privileges
 */
...
// Admin confirm user using AWS CLI
async function adminConfirmUser(username) {
  try {
    log(`Admin confirming user: ${username}`, 'admin');
    
    const command = `aws cognito-idp admin-confirm-sign-up --user-pool-id ${CONFIG.userPoolId} --username ${username} --region ${CONFIG.region}`;
    execSync(command, { encoding: 'utf8' });
    
    recordTest('Admin User Confirmation', true, `User ${username} confirmed`);
    return { success: true };
  } catch (error) {
    recordTest('Admin User Confirmation', false, `Error: ${error.message}`);
...
// Test 4: Test AppSync Integration
async function testAppSyncIntegration(accessToken) {
  try {
    log('Testing AppSync integration with Cognito token', 'info');
    
    const graphqlEndpoint = 'https://imx6fos5lnd3xkdchl4rqtv4pi.appsync-api.eu-west-1.amazonaws.com/graphql';
    ...
    if (response.status === 200 && response.data.data) {
      recordTest('AppSync Integration', true, 'Authenticated request successful');
      return { success: true };
    } else {
      recordTest('AppSync Integration', false, `Status: ${response.status}, Response: ${JSON.stringify(response.data)}`);
      return { success: false };
    }
  } catch (error) {
    recordTest('AppSync Integration', false, `Error: ${error.message}`);
    return { success: false };
  }
}
...
async function runCognitoAuthTests() {
  log('🚀 Starting Cognito Authentication Tests with Admin Confirmation', 'info');
  ...
  if (registrationResult.success && !registrationResult.userExists) {
    // Confirm user using admin privileges
    const confirmResult = await adminConfirmUser(TEST_USER.email);
    ...
    if (confirmResult.success) {
      ...
      if (loginResult.success && loginResult.tokens) {
        ...
        await testAppSyncIntegration(tokens.AccessToken);
      }
    }
    ...
    if (loginResult.success && loginResult.tokens) {
      ...
      await testGetUser(tokens.AccessToken);
      await testAppSyncIntegration(tokens.AccessToken);
    } else if (loginResult.error && loginResult.error.message === 'User is not confirmed.') {
      // Confirm existing user and try again
      const confirmResult = await adminConfirmUser(TEST_USER.email);
      if (confirmResult.success) {
        const retryLoginResult = await testUserLogin();
        if (retryLoginResult.success && retryLoginResult.tokens) {
          tokens = retryLoginResult.tokens;
          await testGetUser(tokens.AccessToken);
          await testAppSyncIntegration(tokens.AccessToken);
        }
      }
  ...
  if (results.passed >= 4) {
    ...
    log('✅ User registration, login, and AppSync integration working', 'success');
    log('🔒 JWT tokens are valid and accepted by AppSync', 'success');
